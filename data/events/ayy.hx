...
function postCreate() {
	for (sl in strumLines.members) {
		...
		for (character in sl.characters) {
			if (!StringTools.contains(character.curCharacter, 'lady') && Assets.exists(Paths.sound('game/ayy/${character.curCharacter}'))) {
				_ayys.push(FlxG.sound.load(Paths.sound('game/ayy/${character.curCharacter}'), 0.75));
			}
		}
