...
function create() {
	var _splashList:Array<String> = [for (fileName in Paths.getFolderContent('images/splashScreens')) Path.withoutExtension(fileName)];
	var _selected:String = _splashList[FlxG.random.int(0, _splashList.length-1)];
	var splash:FunkinSprite = new FunkinSprite().loadSprite(Paths.image('splashScreens/${_selected}'));
